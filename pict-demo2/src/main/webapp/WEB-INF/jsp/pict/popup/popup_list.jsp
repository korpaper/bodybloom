    ...
    <%@include file="../main/navigation.jsp" %>
        <div id="layoutSidenav">
			...
			<div id="layoutSidenav_content">
				<main class="contents">
					<h2 class="contents-title">팝업 리스트</h2>
							        ...
							        <table style="text-align : left">
							            ...
							            <tbody>
								            <c:forEach var="resultList" items="${resultList}" varStatus="status">
								                <tr>
							                    	<td>${status.count}</td>
							                    	<td class="opt-tl"><a href="javascript:void(0);" onclick="popup_mod('${resultList.idx}');" class="link">${resultList.title}</a></td>
							                    	<td>${resultList.linkurl}</td>
							                    	<td>${resultList.regdate}</td>
							                    	<td>
							                    		<button type="button" onclick="javascript:popup_delete('${resultList.idx}')" class="btn-basic btn-fill btn-sm">삭제</button>
									            	</td>
								                </tr>
							                </c:forEach>
							            </tbody>
		            ...
		            <!-- <div style="float : right; margin-right: 20%">
			            <button type="button" id="button1" onclick="popup_list();">게시글 리스트</button>
		            </div> -->
				</main>
			</div>
		</div>
		...
		<script>
			...
			function search(){
				$("#search_fm").attr("action", "/popup/popup_list");
				$("#search_fm").submit();
			}
		</script>
